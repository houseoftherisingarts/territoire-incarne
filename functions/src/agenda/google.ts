...
import { actionPourRdv, evenementDepuisRdv, occupationDepuisRdv, occupationsDepuisFreebusy, type RdvPourEvenement } from "./sync";
import { ADMIN_EMAILS, ADMIN_UIDS } from "../admins";
...
export function assertAdmin(request: CallableRequest): void {
  ...
  if (!auth || (!ADMIN_UIDS.includes(auth.uid) && !emailOk)) {
    throw new HttpsError("permission-denied", "Admin only.");
  }
}

const branche = (): boolean => GOOGLE_OAUTH_CLIENT_ID.value().length > 10 && GOOGLE_OAUTH_CLIENT_SECRET.value().length > 10 && AGENDA_STATE_SECRET.value().length > 10;
const projectId = (): string => process.env.GCLOUD_PROJECT || "territoireincarne-80bb9";
...
export const agendaGoogleConnecter = onCall({ region: REGION, secrets: AGENDA_SECRETS }, async (request) => {
  assertAdmin(request);
  if (!branche()) throw new HttpsError("failed-precondition", "indisponible");
...
export const agendaGoogleRetour = onRequest({ region: REGION, secrets: AGENDA_SECRETS }, async (req, res) => {
  ...
  try {
    ...
    await getFirestore().doc(PRIVE_PATH).set(
      { refreshToken: tokens.refresh_token, email: info.data.email || "", calendrierId: "primary", connectedAt: Timestamp.now(), derniereSync: null },
      { merge: true },
    );
...
export const agendaGoogleEtat = onCall({ region: REGION, secrets: AGENDA_SECRETS }, async (request: CallableRequest<EtatRequete>) => {
  assertAdmin(request);
  if (!branche()) return { disponible: false, connecte: false };
  const ref = getFirestore().doc(PRIVE_PATH);
  const snap = await ref.get();
  if (!snap.exists) return { disponible: true, connecte: false };
...
const rdvDepuisDoc = (d: any): RdvPourEvenement => ({
...
async function synchroniserUnRdv(cal: calendar_v3.Calendar, calendarId: string, db: FirebaseFirestore.Firestore, id: string, rdv: RdvPourEvenement): Promise<void> {
  const geste = actionPourRdv(rdv);
  const ref = db.doc(`appointments/${id}`);
  if (geste.action === "creer") {
    const { data } = await cal.events.insert({ calendarId, requestBody: evenementDepuisRdv(rdv) });
    if (data.id) await ref.update({ googleEventId: data.id });
  } else if (geste.action === "mettreAJour") {
    try {
      await cal.events.update({ calendarId, eventId: geste.eventId, requestBody: evenementDepuisRdv(rdv) });
    } catch (e) {
...
async function synchroniserOccupationsGoogle(cal: calendar_v3.Calendar, calendarId: string, db: FirebaseFirestore.Firestore): Promise<void> {
  ...
  const fraiches = occupationsDepuisFreebusy(data.calendars?.[calendarId]?.busy || []);
  ...
  for (const o of fraiches) batch.set(db.doc(`occupations/${o.id}`), { start: Timestamp.fromDate(o.start), end: Timestamp.fromDate(o.end), source: "google" });
  await batch.commit();
}
...
async function synchroniserTout(db: FirebaseFirestore.Firestore): Promise<void> {
  ...
  for (const doc of rdvs.docs) await synchroniserUnRdv(cal, calendarId, db, doc.id, rdvDepuisDoc(doc.data()));
...
export const agendaSyncSurEcriture = onDocumentWritten({ document: "appointments/{rdvId}", region: REGION, secrets: AGENDA_SECRETS }, async (event) => {
  ...
  const occ = occupationDepuisRdv(id, rdv);
  if (occ) await occRef.set({ start: Timestamp.fromDate(occ.start), end: Timestamp.fromDate(occ.end), source: "rdv" });
  else await occRef.delete().catch(() => undefined);
