...
exports.createCheckoutSession = (0, https_1.onCall)({
    ...
    if (data.purpose === "class" || data.purpose === "event") {
        ...
        if (!docId)
            throw new https_1.HttpsError("invalid-argument", "identifiant manquant");
        const snap = await db.doc(`${coll}/${docId}`).get();
        if (!snap.exists)
            throw new https_1.HttpsError("not-found", "introuvable");
        const d = snap.data();
        if (data.purpose === "class" ? d.active === false : d.published === false) {
            throw new https_1.HttpsError("failed-precondition", "fermé aux inscriptions");
        }
        if (!Number.isInteger(d.priceCents) || d.priceCents <= 0) {
            throw new https_1.HttpsError("failed-precondition", "gratuit : aucun paiement à faire");
        }
        if (d.capacity && d.capacity > 0) {
            ...
            if (payes.data().count >= d.capacity && dejaMembre.data()?.status !== "paid") {
                throw new https_1.HttpsError("resource-exhausted", "complet");
            }
        }
