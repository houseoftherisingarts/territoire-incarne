    ...
    <div className="space-y-16">
      ...
      <Reveal>
        <ul className="border-t border-ink/10 dark:border-white/10">
          {content.contextes.map((c, i) => (
            <li key={i} className="grid grid-cols-1 md:grid-cols-12 gap-x-8 gap-y-2 py-7 border-b border-ink/10 dark:border-white/10">
              <span className="md:col-span-5 font-serif text-2xl md:text-3xl font-light leading-tight text-ink dark:text-stone-100">{c.titre}</span>
              <span className="md:col-span-7 font-serif text-lg leading-relaxed text-ink/65 dark:text-stone-300">{c.texte}</span>
            </li>
          ))}
        </ul>
      </Reveal>
